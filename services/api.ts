...
export const getPitcherPrediction = async (pitcherName: string, date?: string): Promise<PredictionResponse> => {
  try {
    ...
    return {
      ...
      qs_probability: data.qs_probability,
      threshold: 0.5,
...
export const getTopPitchers = async (sortBy: string = 'qs_probability'): Promise<Pitcher[]> => {
  try {
    const response = await fetch(`${API_BASE_URL}/get_top_predictions?sort_by=${sortBy}`);
    if(!response.ok) {
    ...
    return data.map((item: any, index: number) => ({
      ...
      season_era: item.season_era,
      season_whip: item.season_whip,
...
export const getAllPitchers = async (): Promise<Pitcher[]> => {
  try {
    const response = await fetch(`${API_BASE_URL}/get_all_predictions`);
    if(!response.ok) {
      console.warn("Failed to fetch top pitchers, using mock data.");
      return MOCK_TOP_PITCHERS;
    }
